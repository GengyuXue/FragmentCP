...
for (l in 1:iteration) {
  ...
  set.seed(2000+25*l)
  # set.seed(90)
  data1 = temp_fragment_data21(mu = 0, r = 3, sigma=1, n = 200, m = 10, sigma_epsilon = 0.1, domain = c(0, 1), delta = 0.5)
  data2 = temp_fragment_data22(mu = 0, r = 3, sigma=1, n = 200, m = 10, sigma_epsilon = 0.1, domain = c(0, 1), delta = 0.5)
  ...
  if(length(cpt_init) != 1){next}
    ...
    refined_eta_with_truth <- local_refine_fragment_truth(data$t, data$y, data$r, cpt_init, C1, C2,
                                                          ...
                                                          w = 0.75)
    print(c("refined with truth", refined_eta_with_truth))
    
    initial[l] <- cpt_init
    refined[l] <- refined_eta_with_truth
    ...
    # C <- kappa2_list$C
    sigma2_list <- sigma2_fragment(true_change, kappa2, C, data$t, data$y, data$r)
    sigma_before <- sigma2_list[[1]]
    sigma_after <- sigma2_list[[2]]
    drift <- omega_fragment(true_change, kappa2, C, data$t, data$y, data$r)
    ...
    CI_matrix1[l,1] = floor(quantile(d1, probs = alpha1/2)/kappa2) + refined_eta_with_truth
    CI_matrix1[l,2] = ceiling(quantile(d1, probs = 1-alpha1/2)/kappa2) + refined_eta_with_truth
    
    CI_matrix2[l,1] = floor(quantile(d1, probs = alpha2/2)/kappa2) + refined_eta_with_truth
    CI_matrix2[l,2] = ceiling(quantile(d1, probs = 1-alpha2/2)/kappa2) + refined_eta_with_truth
...
save.image(file = "n200_m10.RData")
