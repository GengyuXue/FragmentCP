...
for (l in 1:iteration) {
  print(c("iteration", l))
  set.seed(1000+100*l)
  data1 = temp_fragment_data10(mu = 0, r = 3, sigma=1, n = 200, m = 20, sigma_epsilon = 0.001, domain = c(0, 1), delta = 0.7)
  data2 = temp_fragment_data13(mu = 0, r = 3, sigma=1, n = 200, m = 20, sigma_epsilon = 0.001, domain = c(0, 1), delta = 0.7)
  ...
  if(length(cpt_init) != 1){
    ...
    # estimate jump size
    kappa2_list <-  kappa2_fragment(data$t, data$y, data$r, refined_eta, r=3, lambda, ext = 0.1, maxIt = 1)
    ...
    print(quantile(d, probs = c(alpha/2, 1-alpha/2))/kappa2)
    
    CI_matrix[l,] = quantile(d, probs = c(alpha/2, 1-alpha/2))/kappa2 + refined_eta
    
  }
}
CI_matrix <- na.omit(CI_matrix)

width <- rep(0, nrow(CI_matrix))

true_change <- 200
count <- 0

for(i in 1:nrow(CI_matrix)){
  if (true_change >= CI_matrix[i,1] & true_change <= CI_matrix[i,2]){
    count <- count +1
  }
  width[i] <- CI_matrix[i,2] - CI_matrix[i,1]
}
